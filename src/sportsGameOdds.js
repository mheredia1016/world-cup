...
const allowedBooks = String(
  process.env.BOOKMAKER_IDS || 'fanduel,draftkings,hardrock,bet365'
)
  .split(',')
...
export function getMatchName(event) {
  ...
  return event.activity || event.name || event.eventID || 'Unknown Match';
}
...
function bestBook(odd) {
  ...
  for (const [book, row] of Object.entries(books)) {
    ...
    if (!best || parseAmericanOdds(candidate.odds) > parseAmericanOdds(best.odds)) {
      best = candidate;
    }
  }
...
function isPlayableProp(odd, book) {
  if (!book) return false;

  const type = propType(odd);
  const odds = parseAmericanOdds(book.odds);
